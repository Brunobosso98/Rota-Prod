...
$(document).ready(function() {
    ...
    const csrfToken = window.csrfToken || $('meta[name="csrf-token"]').attr('content') || $('input[name="csrf_token"]').val();
    
    // Base URL para APIs
    const baseURL = window.location.origin;
    
    console.log("Check-in/out: CSRF Token disponível:", !!csrfToken);
    ...
    window.sendCheckInRequest = function(routeId, locationId, latitude, longitude, btn, listItem) {
        ...
        if (!csrfToken) {
            toastr.error('Token CSRF não encontrado. Recarregue a página e tente novamente.');
            return;
        }
        ...
        console.log('Enviando requisição de check-in com dados:', {
            ...requestData,
            csrf_token: csrfToken ? csrfToken.substring(0, 10) + '...' : 'null'
        });
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log('Resposta de check-in recebida:', response);
                if (response.success) {
                    ...
                    if (btn) btn.prop('disabled', false).html('<i class="fas fa-sign-in-alt"></i> Check-in');
                }
            },
            error: function (xhr, status, error) {
                ...
                try {
                    ...
                    if (xhr.status === 403) {
                        errorMsg = 'Erro de autenticação. Tente recarregar a página e fazer login novamente.';
                        ...
                        errorMsg = 'Erro de conexão com o servidor. Verifique sua conexão de internet.';
                    } else {
    ...
    window.sendCheckOutRequest = function(routeId, locationId, btn, listItem) {
        ...
        // Logar para depuração
        console.log('Enviando requisição de check-out');
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log('Resposta de check-out recebida:', response);
                if (response.success) {
                    ...
                    if (btn) btn.prop('disabled', false).html('<i class="fas fa-sign-out-alt"></i> Check-out');
                }
            },
            error: function (xhr, status, error) {
